...
function buildDiff(before,after){const changed={};for(const key of Object.keys(after)){if(!isEqual(before[key],after[key]))changed[key]={before:before[key],after:after[key]};}return changed;}
exports.handler = async (event) => {
  ...
  try {
    ...
    if (!Object.keys(changes).length) return json(200, { ok: true, message: "No changes", changes: {} });
    const configContent = JSON.stringify(after, null, 2) + "\n";
    const configResult = await putRepoFile(configPath, configContent, `update config by ${auth.session.username}`, oldConfigFile.sha);
    let oldLogSha = null, oldLogs = "";
    try { const logFile = await getRepoFile(logPath); oldLogSha = logFile.sha; oldLogs = logFile.content || ""; } catch (err) { oldLogs = ""; }
    const logLine = JSON.stringify({ time: new Date().toISOString(), user: auth.session.username, role: auth.session.role || "user", ip: auth.session.ip || "", deviceInfo: auth.session.deviceInfo || "", action: "update_config", changes });
    const newLogs = oldLogs ? `${oldLogs.trimEnd()}\n${logLine}\n` : `${logLine}\n`;
    await putRepoFile(logPath, newLogs, `append config log by ${auth.session.username}`, oldLogSha || undefined);
    return json(200, { ok: true, commitSha: configResult.commit?.sha || "", changes });
  } catch (err) {
